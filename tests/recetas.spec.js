...
describe("Endpoints recetas", () => {
  describe("GET /hradb-a-mongodb/recetas/ultimo/:codigoEstablecimiento/:tipoReceta", () => {
    // test autorizacion
    it("Should not get last receta from database", async (done) => {
      // ejecutar endpoint
      const response = await request
        .get("/hradb-a-mongodb/recetas/ultimo/5")
  ...
  describe("POST /hradb-a-mongodb/recetas", () => {
    // test autorizacion
    it("Should not save receta to database", async (done) => {
      // ejecutar endpoint
      const response = await request
        .post("/hradb-a-mongodb/recetas")
        .set("Authorization", "no-token")
        .send(recetaGuardar);
